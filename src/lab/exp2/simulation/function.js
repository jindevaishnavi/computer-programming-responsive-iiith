window.view = {
	...
	validateRightangledTriangleInputs: function() {
		...
		if ( a !== '2')
			alert('Incorrect value of input variables(arguments). Calculating the area of a right angled triangle requires the base and height of the triangle. Try again.')
		else if ( b !== 'float' )
			alert('Incorrect datatype of input variables(arguments). The value of the base of the triangle need not be integers. Try again.')
		else if ( c !== 'float' )
	...
	displayFunctionForRightAngledTriangle: function() {
		document.getElementById('functionRightangledTriangle').innerHTML += 'float area_right_angled_triangle (float a,float b)<br>{<br> &emsp; float area = (1/2)*a*b;<br> &emsp; return area;<br>}'
	},
	...
	area_right_angled_triangle: function( param, functionCall ) {
	...
	calculateArea: function() {
		...
		for ( i = 0 ; i < elements.length ; i ++ ) {
			...
			else if ( parameter.length === 2 && functionCall === 'area_right_angled_triangle()' && isNaN(parameter[0]) === false && isNaN(parameter[1]) === false )
				this.area_right_angled_triangle(parameter, parsedValue)
			else
		 	alert( 'Incorrect function call at line ' + String( i + 1 ) )
		}	
